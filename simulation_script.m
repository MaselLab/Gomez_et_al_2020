...
outputfile = '~/Documents/mutBiasCI/data/TimeSeries/mutBiasCI_data_for_2d_distribution_ml-100'; 
...
% init_filename = '~/Documents/mutBiasCI/data/TimeSeries/mutBiasCI_data_for_2d_distribution_ml-100';
...
for i=1:data_pts_s
    for j=i:data_pts_s
        indx = indx + 1
        if (j >= 10) % simulations with one trait in origin-fixation must run for 2 million generations to converge.
            end_time(indx)=2*end_time(indx);
        end
        ...
        if ( sum(init_indx == indx) == 1 )
            ...
            [init_time,init_param,init_pop,init_fit,init_fitx,init_fity] = ...
                get_init_population_data(input_file_param,input_file_class,input_file_abund,input_file_summr);
        end
        ...
        [sim_data(indx,1),sim_data(indx,2),sim_data(indx,3),sim_data(indx,4),sim_data(indx,5),sim_data(indx,6)] ...
            = stochastic_simulation_two_traits(N,sarry(i),Uarry(i),sarry(j),Uarry(j),Uarry(i),Uarry(j),steps, ...
                ...
                init_flag,init_time,init_pop,init_fit,init_fitx,init_fity);
    end
    
end
toc

dlmwrite('~/Documents/mutBiasCI/data/TwoTraitSim/mutBiasCI_data_all_simulation_parameters_ml-100-0.dat',NsU,'delimiter',',','precision',16);
dlmwrite('~/Documents/mutBiasCI/data/TwoTraitSim/mutBiasCI_data_all_simulation_grand_means_ml-100-1.dat',sim_data,'delimiter',',','precision',16);
dlmwrite('~/Documents/mutBiasCI/data/TwoTraitSim/mutBiasCI_data_all_simulation_indx_of_collected_data_ml-100-2.dat',indx_of_collected_data,'delimiter',',','precision',16);
